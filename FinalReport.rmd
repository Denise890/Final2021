...
```{r}
Mals <- maledata %>% group_by(treatment,infected) %>% summarize(count=n()) %>% mutate(prcnt=count/sum(count))
...
As seen on the table above there were thousands of amles involved in the study and out of all those people 340 males became infected with Covid
